...
import { ModelConfig, redactModelConfigSecrets } from '@/components/ModelSettingsModal';
...
const Sidebar: React.FC = () => {
  ...
  useEffect(() => {
    const setupListener = async () => {
      ...
      const unlisten = await listen<ModelConfig>('model-config-updated', (event) => {
        console.log('Sidebar received model configuration metadata:', { provider: event.payload.provider, model: event.payload.model });
  ...
  const handleSaveModelConfig = async (config: ModelConfig) => {
    try {
      ...
      const safeConfig = redactModelConfigSecrets(config);
      setModelConfig(safeConfig);
      console.log('Model config saved successfully');
      setSettingsSaveSuccess(true);

      // Emit only non-secret metadata to other WebView listeners.
  ...
  const handleSaveTranscriptConfig = async (updatedConfig?: TranscriptModelProps) => {
    try {
      ...
      console.log('Saving transcript configuration:', { provider: payload.provider, model: payload.model, apiKeyConfigured: Boolean(payload.apiKey) });
      ...
      setTranscriptModelConfig({
        ...configToSave,
        apiKey: null,
        apiKeyConfigured: Boolean(configToSave.apiKeyConfigured || configToSave.apiKey),
      });
      setSettingsSaveSuccess(true);
