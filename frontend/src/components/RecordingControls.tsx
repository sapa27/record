    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        ...
        setDeviceError({
          ...
          message: 'ไม่สามารถบันทึกเสียงระบบได้ โปรดตรวจว่า:\n• เลือกอุปกรณ์เสียงระบบหรืออุปกรณ์ Loopback/What U Hear แล้ว\n• อุปกรณ์เล่นเสียงของ Windows เปิดใช้งานอยู่\n• ไม่มีโปรแกรมอื่นผูกขาดอุปกรณ์เสียง'
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: 'Permission Required',
          message: 'ต้องอนุญาตการบันทึกเสียง โปรด:\n• เปิด Windows Settings > Privacy & security > Microphone\n• อนุญาตให้แอปเดสก์ท็อปเข้าถึงไมโครโฟน\n• ปิดและเปิดโปรแกรมใหม่หลังเปลี่ยนสิทธิ์'
        });
      } else {
        setDeviceError({
    ...
    const setupListeners = async () => {
      try {
        ...
        const transcriptErrorUnsubscribe = await listen('transcript-error', (event) => {
          console.warn('Transcript error event received');
          const errorMessage = event.payload as string;

          Analytics.trackTranscriptionError(errorMessage);
          console.log('Tracked transcription error category');
        ...
        const transcriptionErrorUnsubscribe = await listen('transcription-error', (event) => {
          console.warn('Structured transcription error event received');
