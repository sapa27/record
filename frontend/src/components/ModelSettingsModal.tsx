...
export interface ModelConfig {
  ...
  customOpenAIApiKeyConfigured?: boolean;
  maxTokens?: number | null;
  temperature?: number | null;
  topP?: number | null;
}

export function redactModelConfigSecrets(config: ModelConfig): ModelConfig {
  return {
    ...config,
    apiKey: null,
    customOpenAIApiKey: null,
    apiKeyConfigured: Boolean(config.apiKeyConfigured || config.apiKey?.trim()),
    customOpenAIApiKeyConfigured: Boolean(
      config.customOpenAIApiKeyConfigured || config.customOpenAIApiKey?.trim()
    ),
  };
}
...
export function ModelSettingsModal({
  ...
  const [isApiKeyLocked, setIsApiKeyLocked] = useState<boolean>(Boolean(modelConfig.apiKeyConfigured || modelConfig.apiKey?.trim()));
  ...
  const isDoneDisabled =
    (requiresApiKey && !modelConfig.apiKeyConfigured && (!apiKey || (typeof apiKey === 'string' && !apiKey.trim()))) ||
          ...
          if (data.provider === 'custom-openai') {
            try {
              ...
              if (customConfig) {
                ...
                setModelConfig((previous: ModelConfig) => ({ ...previous, customOpenAIApiKeyConfigured: Boolean(customConfig.apiKeyConfigured) }));
  ...
  const handleSave = async () => {
    ...
    const redactedConfig = redactModelConfigSecrets(updatedConfig);
    setModelConfig(redactedConfig);
    console.log('Model settings prepared for secure persistence:', {
      provider: redactedConfig.provider,
      model: redactedConfig.model,
      apiKeyConfigured: redactedConfig.apiKeyConfigured,
      customApiKeyConfigured: redactedConfig.customOpenAIApiKeyConfigured,
    });
    ...
    if (updateProviderApiKey && updatedConfig.provider !== 'custom-openai') {
      // Never retain entered credentials in React context after persistence.
      updateProviderApiKey(updatedConfig.provider, null);
    }
    setApiKey(null);
    setCustomOpenAIApiKey('');

    // Pass the one-time credential only to the persistence callback. Every
    // state/event update after the save must use redactModelConfigSecrets().
