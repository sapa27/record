...
export function RetranscribeDialog({
  ...
  const [selectedLang, setSelectedLang] = useState(selectedLanguage || 'th');
  ...
  useEffect(() => {
    ...
    if (open && !wasOpen) {
      ...
      setProgress(null);
      setError(null);
      setSelectedLang(selectedLanguage || 'th');
  ...
  const handleStartRetranscription = async () => {
    ...
    try {
      const languageToSend = isParakeetModel ? 'auto' : selectedLang;
