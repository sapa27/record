...
function isWhisperModelAvailable(models: Array<{ name?: string; status?: unknown }>): boolean {
...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Listen to the required multilingual Whisper model download progress
  useEffect(() => {
  ...
  const verifyModelStatus = async (savedStatus: OnboardingStatus) => {
    ...
    // Verify the required multilingual Whisper model exists on disk
    try {
      await invoke('whisper_init');
      const models = await invoke<Array<{ name?: string; status?: unknown }>>('whisper_get_available_models');
      parakeetDownloaded = isWhisperModelAvailable(models);
      console.log('[OnboardingContext] Required Whisper model verified on disk:', parakeetDownloaded);
    } catch (error) {
    ...
    // Windows flow: Step 1 Welcome, Step 2 Setup Overview, Step 3 Download Progress
    let currentStep = savedStatus.current_step;
    let completed = savedStatus.completed;

    // Clamp legacy onboarding state to the Windows flow.
    if (currentStep > 3) {
  ...
  const startBackgroundDownloads = async ({
    ...
    try {
      ...
      // Start the multilingual Whisper download first (speech recognition - required for Thai)
      if (shouldStartParakeet) {
        console.log('[OnboardingContext] Starting multilingual Whisper download');
        invoke('whisper_init').then(() => invoke('whisper_download_model', { modelName: REQUIRED_TRANSCRIPTION_MODEL }))
  ...
  const checkActiveDownloads = async () => {
    try {
      ...
      const models = await invoke<Array<{ name?: string; status?: unknown }>>('whisper_get_available_models');
      const required = models.find((model) => model.name === REQUIRED_TRANSCRIPTION_MODEL);
      const isDownloading = required?.status === 'Downloading'
        || (!!required?.status && typeof required.status === 'object' && 'Downloading' in required.status);
      ...
      // Built-in summary model downloads are tracked by their own progress event.
  ...
  const retryParakeetDownload = async () => {
    console.log('[OnboardingContext] Retrying multilingual Whisper download');
    try {
      await invoke('whisper_init');
  ...
  const goNext = useCallback(() => {
    setCurrentStep((prev: number) => {
      const next = prev + 1;
      // The Windows setup contains three steps.
