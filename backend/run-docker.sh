...
START OPTIONS:
  -m, --model MODEL        Whisper model to use (default: large-v3-turbo-q5_0)
  ...
  --language LANG         Language code (default: th)
...
select_model() {
    local default_model="${1:-large-v3-turbo-q5_0}"
...
select_language() {
    ...
    show_option "1" "th" "th (Thai)"
    show_option "2" "auto" "auto (automatic detection)"
    show_option "3" "en" "en (English)"
    show_option "4" "es" "es (Spanish)"
    show_option "5" "fr" "fr (French)"
    show_option "6" "de" "de (German)"
    show_option "7" "it" "it (Italian)"
    show_option "8" "pt" "pt (Portuguese)"
    show_option "9" "ru" "ru (Russian)"
    show_option "10" "ja" "ja (Japanese)"
    show_option "11" "zh" "zh (Chinese)"
    echo " 12) Other (enter language code)" >&2
    echo >&2
        ...
        case "$choice" in
            ...
            *)
                ...
                if [[ "$choice" =~ ^[a-z]{2}$ ]]; then
                    ...
                    echo -e "${RED}Invalid selection. Please choose 1-12 or enter a valid language code.${NC}" >&2
                fi
                ;;
        esac
    done
...
manage_models() {
    ...
    case "$action" in
        "list")
            ...
            ;;
        "download")
            local model_name="${2:-large-v3-turbo-q5_0}"
            local models_dir=$(ensure_models_dir)
