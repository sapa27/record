...
class DatabaseManager:
    ...
    def _legacy_init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # P0 hardening: remove all credentials previously stored in plaintext.
            # The legacy HTTP backend no longer stores cloud credentials; the
            # desktop application uses the operating-system credential store.
            cursor.execute("""
                UPDATE settings
                SET groqApiKey = NULL,
                    openaiApiKey = NULL,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if not existing_meeting:
                    ...
                    logger.info(f"Saved meeting metadata for meeting_id: {meeting_id}; folder_configured={bool(folder_path)}")
                else:
    ...
    async def save_api_key(self, api_key: str, provider: str):
        """Legacy plaintext credential storage is disabled."""
        raise RuntimeError(
            "The legacy backend does not store API keys. Configure credentials in the desktop OS credential store."
        )

    async def get_api_key(self, provider: str):
        """Legacy plaintext credentials are intentionally unavailable."""
        return ""
    ...
    async def save_transcript_api_key(self, api_key: str, provider: str):
        """Legacy plaintext transcription credential storage is disabled."""
        raise RuntimeError(
            "The legacy backend does not store transcription API keys. Use local Whisper."
        )

    async def get_transcript_api_key(self, provider: str):
        """Legacy plaintext transcription credentials are unavailable."""
        return ""
    ...
    async def delete_api_key(self, provider: str):
        """Credentials are not stored by the legacy backend."""
        return None
