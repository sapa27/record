...
log_info "Setting up hardened local-service credentials..."
cd ../.. || handle_error "Failed to return to project root"
if [ ! -f backend/.env ]; then
    python3 backend/generate_security_env.py --output backend/.env || handle_error "Failed to generate protected environment file"
else
    log_info "backend/.env already exists; preserving existing credentials."
fi
log_info "Cloud AI credentials are intentionally not stored in the backend .env file."
