...
echo Setting up hardened local-service credentials...
if not exist ".env" (
    python generate_security_env.py --output .env
    if errorlevel 1 (
        echo Failed to generate protected environment file.
        exit /b 1
    )
) else (
    echo .env already exists. Skipping generation...
)
echo Cloud AI credentials are intentionally not stored in the backend .env file.
