...
int main(int argc, char ** argv) {
    ...
    const char * token_environment = std::getenv("MEETILY_WHISPER_TOKEN");
    const std::string api_token = token_environment == nullptr ? "" : token_environment;
    if (api_token.size() < 32) {
        fprintf(stderr, "[SECURITY] MEETILY_WHISPER_TOKEN must contain at least 32 characters. Server refused to start.\n");
        return 4;
    }

    const bool container_mode = std::getenv("MEETILY_CONTAINER_MODE") != nullptr &&
                                std::string(std::getenv("MEETILY_CONTAINER_MODE")) == "1";
    if (sparams.hostname != "127.0.0.1" && sparams.hostname != "localhost" &&
        !(container_mode && sparams.hostname == "0.0.0.0")) {
        fprintf(stderr, "[SECURITY] Refusing non-loopback bind. Use Docker loopback publishing with MEETILY_CONTAINER_MODE=1 when required.\n");
        return 4;
    }
    ...
    // Every route except the minimal health check requires a strong bearer token.
    svr.set_pre_routing_handler([&api_token](const Request & req, Response & res) {
        if (req.path == "/healthz") {
            return Server::HandlerResponse::Unhandled;
        }
        const std::string authorization = req.get_header_value("Authorization");
        const std::string expected = "Bearer " + api_token;
        if (!constant_time_equals(authorization, expected)) {
            res.status = 401;
    ...
    // The hardened server exposes API routes only; embedded/static web UI is disabled.
    ...
    // Streaming buffers are isolated by authenticated client session.
    const int MIN_AUDIO_LENGTH_MS = 1000;
    const size_t MAX_STREAM_SESSIONS = 32;
    const size_t MAX_STREAM_SAMPLES = 16000 * 60; // 60 seconds per request/session safety cap
    std::unordered_map<std::string, std::vector<float>> audio_buffers;

    // Explicitly release a streaming session. This endpoint is authenticated by
    // the global pre-routing bearer-token guard.
    svr.Post(sparams.request_path + "/stream/reset", [&](const Request &req, Response &res) {
        std::lock_guard<std::mutex> lock(whisper_mutex);
        const std::string session_id = req.get_header_value("X-Meetily-Session");
        if (!valid_session_id(session_id)) {
            res.status = 400;
            res.set_content("{\"error\":\"valid X-Meetily-Session header required\"}", "application/json");
            return;
        }
        const size_t erased = audio_buffers.erase(session_id);
        json response = {{"status", "reset"}, {"released", erased == 1}};
    ...
    svr.Post(sparams.request_path + "/stream", [&](const Request &req, Response &res) {
        ...
        if (audio_file.content.size() % sizeof(float) != 0) {
            res.status = 400;
            res.set_content("{\"error\":\"invalid audio payload\"}", "application/json");
            return;
        }
        const float* audio_data = reinterpret_cast<const float*>(audio_file.content.data());
        const size_t n_samples = audio_file.content.size() / sizeof(float);
        if (n_samples == 0 || n_samples > MAX_STREAM_SAMPLES) {
            res.status = 413;
            res.set_content("{\"error\":\"audio chunk exceeds safety limit\"}", "application/json");
            return;
        }

        auto buffer_iterator = audio_buffers.find(session_id);
        if (buffer_iterator == audio_buffers.end()) {
            if (audio_buffers.size() >= MAX_STREAM_SESSIONS) {
                res.status = 429;
                res.set_content("{\"error\":\"too many active streaming sessions\"}", "application/json");
                return;
            }
            buffer_iterator = audio_buffers.emplace(session_id, std::vector<float>{}).first;
        }
        std::vector<float> & audio_buffer = buffer_iterator->second;
        if (audio_buffer.size() + n_samples > MAX_STREAM_SAMPLES) {
            audio_buffer.clear();
            res.status = 413;
            res.set_content("{\"error\":\"stream buffer exceeds safety limit\"}", "application/json");
            return;
        }
        ...
        const std::string stream_end = req.get_header_value("X-Meetily-Stream-End");
        const bool release_session = stream_end == "1" || stream_end == "true" || stream_end == "TRUE";
        if (release_session) {
            audio_buffers.erase(session_id);
            response["buffer_size_ms"] = 0;
            response["session_released"] = true;
        } else {
    ...
    // Dynamic model loading is intentionally disabled in production.

    svr.set_exception_handler([](const Request &, Response &res, std::exception_ptr ep) {
        try {
            std::rethrow_exception(std::move(ep));
        } catch (const std::exception & exception) {
            fprintf(stderr, "[ERROR] Unhandled request exception: %s\n", exception.what());
        } catch (...) {
            fprintf(stderr, "[ERROR] Unhandled non-standard request exception\n");
        }
