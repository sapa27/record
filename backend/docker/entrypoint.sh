...
WHISPER_MODEL=${WHISPER_MODEL:-models/ggml-large-v3-turbo-q5_0.bin}
...
ensure_model() {
    ...
    # Extract model size from filename (e.g., ggml-large-v3-turbo-q5_0.bin -> large-v3-turbo-q5_0)
    local model_size=""
    ...
    if [[ "$model_size" != "large-v3-turbo-q5_0" ]]; then
        log_warn "🔄 Attempting fallback to large-v3-turbo-q5_0 multilingual model..."
        local fallback_path="models/ggml-large-v3-turbo-q5_0.bin"
        local fallback_url="https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-large-v3-turbo-q5_0.bin"
        
        if download_model_with_progress "$fallback_path" "$fallback_url" "large-v3-turbo-q5_0"; then
            log_info "✅ Fallback model downloaded successfully!"
            log_warn "⚠️  Using large-v3-turbo-q5_0 instead of requested $model_size"
...
Environment Variables:
  WHISPER_MODEL          Model path (default: models/ggml-large-v3-turbo-q5_0.bin)
  ...
  WHISPER_LANGUAGE       Language code (default: th)
  ...
  MEETILY_WHISPER_TOKEN  Required bearer token (minimum 32 characters)

Examples:
  # Start with custom model
  docker run --env-file .env -e WHISPER_MODEL=models/ggml-large-v3.bin whisper-server
  
  # Start with port mapping
  docker run --env-file .env -p 127.0.0.1:8178:8178 whisper-server
  
  # Start with volume for models
  docker run --env-file .env -v /path/to/models:/app/models whisper-server
