    ...
    if (-not $DryRun -and -not $Push) {
        ...
        if (Test-Path $runScriptPath) {
            ...
            $defaultModel = switch ($BuildType) {
                'gpu' { 'models/ggml-large-v3-turbo-q5_0.bin' }
                'cpu' { 'models/ggml-large-v3-turbo-q5_0.bin' }
                'macos' { 'models/ggml-large-v3-turbo-q5_0.bin' }
                'both' { 'models/ggml-large-v3-turbo-q5_0.bin (CPU/GPU)' }
                default { 'models/ggml-large-v3-turbo-q5_0.bin' }
            }
            Write-Info "   Model: $(if ($env:WHISPER_MODEL) { $env:WHISPER_MODEL } else { $defaultModel })"
            Write-Info "   Host: $(if ($env:WHISPER_HOST) { $env:WHISPER_HOST } else { '127.0.0.1' })"
            ...
            Write-Info "   Language: $(if ($env:WHISPER_LANGUAGE) { $env:WHISPER_LANGUAGE } else { 'th' })"
